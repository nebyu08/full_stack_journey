...
app.get(`/filter/`,(req,res)=>{
  ...
  const joke=jokes.filter((obj) => obj.jokeType  === jokeType);
  res.json(joke);
})

//4. POST a new joke
app.post('/jokes',async(req,res)=>{
 // const maxId=jokes.reduce((max,joke)=>Math.max(max,joke.id),0);
  ...
  console.log(`this is the new created: ${newJoke} `);
  res.json(newJoke);
})
...
app.put('/jokes/:id',(req,res)=>{
  ...
  const joke= jokes.find((joke)=>joke.id=theId)
...
app.delete('/jokes/:id',(req,res)=>{
  const theId=parseInt(req.params.id);
  jokes.splice(theId,1);
    try{
    res.json('ok');
  }
  catch(error){
    console.error('the error has been removed',error);
  }
})
...
app.delete('/all',(req,res)=>{
 ...
 //   jokes.splice(1,jokes.length);
    jokes=[];
    res.json('deleted all mate');
  }else{
    res.status(404).json({error:"you are not authorized to perform this operation"})
    }
})
